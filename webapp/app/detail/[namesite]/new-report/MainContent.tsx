...
export default function MainContent({ responses, onResponsesChange, siteName, currentUser }: MainContentProps) {
  ...
  //generated by ChatGPT, cleans up object URLs to prevent memory leaks
   useEffect(() => {
    return () => {
  ...
  const renderQuestionInput = (question: Question) => {
    ...
    switch (questionType) {
      ...
      case 'image': {
        ...
        const images: ImageWithMeta[] = Array.isArray(response) ? (response as ImageWithMeta[]) : [];


        // Add new files instead of replacing
        const addFiles = (newFiles: File[]) => {
          const newImages: ImageWithMeta[] = newFiles.map((file) => ({
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <input
                          ...
                          value={image.caption}
                          onChange={(e) =>
                            updateImageField(image.id, "caption", e.target.value)
                          }
                          placeholder="Caption (optional)"
                          className="w-full px-3 py-2 border-2 border-[#E4EBE4] rounded-lg focus:border-[#356B43] focus:outline-none transition-colors text-[#254431]"
                        />

                        <textarea
                          value={image.description}
                          onChange={(e) =>
                            updateImageField(image.id, "description", e.target.value)
                          }
                          placeholder="Description (optional)"
                          rows={2}
                          className="w-full px-3 py-2 border-2 border-[#E4EBE4] rounded-lg focus:border-[#356B43] focus:outline-none transition-colors text-[#254431] resize-none"
                        />
