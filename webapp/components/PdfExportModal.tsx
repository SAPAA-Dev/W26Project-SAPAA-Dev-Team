...
export default function PdfExportModal({
  ...
  // Extract unique section titles from inspections
  ...
  // Filtered inspection count preview
  ...
  const toggleResponseId = (id: number) => {
    setOptions((prev) => {
      ...
      return { ...prev, selectedResponseIds: ids };
    });
  };
  ...
  const toggleSection = (section: string) => {
    setOptions((prev) => {
      if (prev.selectedSections === 'all') {
        return {
          ...prev,
          selectedSections: availableSections.filter((s) => s !== section),
        };
      }
      const sections = prev.selectedSections as string[];
      if (sections.includes(section)) {
        const next = sections.filter((s) => s !== section);
        return { ...prev, selectedSections: next.length === 0 ? 'all' : next };
      }
      const next = [...sections, section];
      return {
        ...prev,
        selectedSections:
          next.length === availableSections.length ? 'all' : next,
      };
    });
  };
    ...
    >
      <div className="bg-white rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          {mode !== 'single' && (
            <div className="bg-[#F7F2EA] rounded-2xl p-5 border-2 border-[#E4EBE4]">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-semibold text-[#7A8075] uppercase tracking-wide mb-1 block">
                    From
                  </label>
                  <input
                    type="date"
          ...
          {/* Inspection Selection (site mode only) */}
            ...
            <div className="bg-[#F7F2EA] rounded-2xl p-5 border-2 border-[#E4EBE4]">
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                {inspections.map((r) => {
                  const isSelected =
                    options.selectedResponseIds.length === 0 ||
                    options.selectedResponseIds.includes(r.id);
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <button
              ...
              className={`flex items-center gap-3 p-4 rounded-2xl border-2 transition-all text-left ${
                options.includeImages
                  ? 'bg-[#E4EBE4] border-[#356B43] shadow-sm'
                  : 'bg-white border-[#E4EBE4] hover:border-[#86A98A]'
              }`}
            >
              <div
                className={`w-9 h-9 rounded-lg flex items-center justify-center ${
                  options.includeImages ? 'bg-[#356B43]' : 'bg-[#E4EBE4]'
                }`}
              >
              ...
              <div>
                ...
                <p className="text-xs text-[#7A8075]">
                  {options.includeImages ? `Max ${options.maxImagesPerInspection}/inspection` : 'Off'}
                </p>
              </div>
            </button>
            ...
            >
              <div
                className={`w-9 h-9 rounded-lg flex items-center justify-center ${
                  options.includeCoverPage ? 'bg-[#356B43]' : 'bg-[#E4EBE4]'
                }`}
              >
            ...
            {/* Naturalness Summary Toggle */}
            <button
              onClick={() =>
                setOptions((p) => ({
                  ...p,
                  includeNaturalnessSummary: !p.includeNaturalnessSummary,
                }))
              }
              className={`flex items-center gap-3 p-4 rounded-2xl border-2 transition-all text-left ${
            ...
            >
              <div
                className={`w-9 h-9 rounded-lg flex items-center justify-center ${
                  options.includeNaturalnessSummary ? 'bg-[#356B43]' : 'bg-[#E4EBE4]'
                }`}
              >
                <span
                  className={`text-sm font-bold ${
                    options.includeNaturalnessSummary ? 'text-white' : 'text-[#7A8075]'
                  }`}
                >
                  N
                </span>
            ...
            {/* Empty Answers Toggle */}
            <button
              onClick={() =>
                setOptions((p) => ({ ...p, includeEmptyAnswers: !p.includeEmptyAnswers }))
              }
              className={`flex items-center gap-3 p-4 rounded-2xl border-2 transition-all text-left ${
            ...
            >
              <div
                className={`w-9 h-9 rounded-lg flex items-center justify-center ${
                  options.includeEmptyAnswers ? 'bg-[#356B43]' : 'bg-[#E4EBE4]'
                }`}
              >
                <span
                  className={`text-sm font-bold ${
                    options.includeEmptyAnswers ? 'text-white' : 'text-[#7A8075]'
                  }`}
                >
                  ?
                </span>
              ...
              <div>
                ...
                <p className="text-xs text-[#7A8075]">
                  {options.includeEmptyAnswers ? 'Show all fields' : 'Only answered'}
                </p>
              </div>
            </button>
          </div>
          ...
          <button
            ...
            className="w-full flex items-center justify-between px-4 py-3 bg-[#F7F2EA] rounded-xl border-2 border-[#E4EBE4] hover:border-[#86A98A] transition-all"
          >
            <div className="flex items-center gap-2">
              <Settings className="w-4 h-4 text-[#356B43]" />
              <span className="text-sm font-semibold text-[#254431]">Advanced Options</span>
            </div>
            {showAdvanced ? (
          ...
          {showAdvanced && (
            <div className="space-y-4 pl-2 border-l-2 border-[#E4EBE4]">
              {/* Sort Order */}
              <div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setOptions((p) => ({
                        ...p,
                        maxImagesPerInspection: parseInt(e.target.value),
                      }))
                    }
              ...
              {availableSections.length > 0 && (
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button
                      onClick={() =>
                        setOptions((p) => ({ ...p, selectedSections: 'all' }))
                      }
                      className="text-xs font-semibold text-[#356B43] hover:text-[#254431]"
                    >
                      Select all
                    </button>
                  </div>
                      ...
                      <button
                        ...
                        className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-left transition-all text-sm ${
                          isSectionSelected(section)
                            ? 'bg-[#E4EBE4] text-[#254431] font-medium'
                            : 'bg-white text-[#7A8075] hover:bg-[#F7F2EA]'
                        }`}
                      >
                        {isSectionSelected(section) ? (
                          <CheckSquare className="w-3.5 h-3.5 text-[#356B43] flex-shrink-0" />
                        ) : (
                          <Square className="w-3.5 h-3.5 text-[#7A8075] flex-shrink-0" />
                        )}
                        {section}
                      </button>
                    ))}
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                disabled={loading || (mode !== 'single' && previewCount === 0)}
